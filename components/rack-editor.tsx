...
export function RackEditor({ cinema, onBack, onSave }: RackEditorProps) {
  ...
  const selectedComponent = editedCinema.rackComponents.find(c => c.id === selectedComponentId) || null

  const handleCinemaChange = (field: keyof Cinema, value: any) => {
    setEditedCinema((prev) => ({ ...prev, [field]: value }))
  }
  ...
  const getComponentIcon = (type: string) => {
    switch (type) {
      ...
      case "patch-panel": case "switch": case "router": case "firewall": case "wireless-controller": case "converter":
        return <Network className="h-5 w-5 text-green-600" />
      case "ups": return <Zap className="h-5 w-5 text-purple-600" />
      default: return <Server className="h-5 w-5 text-gray-600" />
    }
  }

  const handleSaveRack = () => {
    const updatedCinema = { ...editedCinema, lastUpdated: new Date().toISOString().split("T")[0] }
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={e => handleComponentChange(selectedComponent.id, "loadPercentage", Number(e.target.value))}
                          />
                        </div>
                      </div>
                      <div>
